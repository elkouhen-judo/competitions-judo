...
L'écran d'accueil liste les compétitions visibles par l'utilisateur. Un judoka standard ne voit que les compétitions dans lesquelles il possède au moins un combat rattaché à son `id_judoka`. Un admin voit toutes les compétitions du classeur. Chaque compétition est affichée sous forme de carte avec son nom, sa date et son lieu. Un appui sur une carte ouvre le détail de la compétition. Les compétitions sont triées par date décroissante afin de faire remonter les événements les plus récents.

Un utilisateur peut créer une compétition en renseignant un nom, une date et un lieu. Une compétition existante peut également être modifiée depuis son écran de détail. Après création ou modification, l'application recharge les données et ouvre la compétition concernée. Les admins peuvent supprimer une compétition. Cette suppression demande confirmation et supprime également tous les combats rattachés afin d'éviter des lignes orphelines dans l'onglet `Combats`.
