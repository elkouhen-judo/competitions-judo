...
test("supabase schema provides an invitation-only signup hook for auth", () => {
  assert.match(schema, /create or replace function public\.hook_check_invited_signup\(event jsonb\)/i);
  ...
  assert.match(schema, /grant execute on function public\.hook_check_invited_signup\(jsonb\) to supabase_auth_admin/i);
  ...
  assert.match(schema, /create policy "Supabase auth admin read judokas for signup hook"[\s\S]*for select[\s\S]*to supabase_auth_admin[\s\S]*using\s*\(\s*true\s*\)/i);
  assert.match(schema, /create policy "Supabase auth admin read invitations for signup hook"[\s\S]*for select[\s\S]*to supabase_auth_admin[\s\S]*using\s*\(\s*true\s*\)/i);
});
